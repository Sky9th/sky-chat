...
public class MainUIController : MonoBehaviour
{
    ...
    private void setMessageItem (Message message)
    {
        newItem = new VisualElement();
    ...
    {
        foreach (KeyValuePair<string, List<Message>> p in data.msgList)
        {
            for (int i = 0; i < p.Value.Count; i++)
            {
                Message message = p.Value[i];
